var maxEC=[0,11,11,11,11,11,10,11,10,10,10,5,5];
for(var i=0;i<13;i++){
...
function getECGoal(name,completed){
	...
	if(name=="eterc5"&&completed==10)return new Decimal("1e750000");
...
function eterc2Mult(){
	...
	if(c<=11)return player.infinityPower.pow(0.002+0.001*c).min(Decimal.pow(10,25*Math.pow(2,c-2)-100)).plus(1);
}
function eterc4Mult(){
