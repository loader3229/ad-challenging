...
function DimensionPower(tier) {
  ...
  if (ECTimesCompleted("eterc2") !== 0 && tier == 1) mult = mult.times(eterc2Mult())
  if (player.currentEternityChall == "eterc2") mult = mult.times(0)

  if (ECTimesCompleted("eterc4") !== 0) mult = mult.times(eterc4Mult())

  if (ECTimesCompleted("eterc9") !== 0) mult = mult.times(eterc9Mult())
...
function buyManyInfinityDimension(tier) {
  ...
  if (ECTimesCompleted("eterc12")) {
      dim.cost = Decimal.round(dim.cost.times(Math.pow(infCostMults[tier], EC12Reward())))
  } else {
...
function buyMaxInfDims(tier) {
  ...
  let costMult;
  if (ECTimesCompleted("eterc12")) {
      costMult = Math.pow(infCostMults[tier], EC12Reward())
  } else {
