...
function getGalaxyCostScalingStart() {
	...
	if(player.galaxies > 1399+eterc5R3()){ // dark matter galaxies
		n -= Math.ceil((player.galaxies-1399-eterc5R3())/5)
	}
    return n
}
...
function updateMetaDimensions () {
 	...
 	var metaShiftRequirement = getMetaShiftRequirement()
         document.getElementById("metaResetLabel").textContent = 'Meta-Dimension ' + (isMetaShift ? "Shift" : "Boost") + ' ('+ player.meta.resets +'): requires ' + metaShiftRequirement.amount + " " + DISPLAY_NAMES[metaShiftRequirement.tier] + " Meta Dimensions"
...
function updateDimensions() {
    if (document.getElementById("antimatterdimensions").style.display == "block" && document.getElementById("dimensions").style.display == "block") {
        ...
        else if (player.resets < getSupersonicReq())document.getElementById("resetLabel").textContent = 'Dimension Boost ('+ player.resets +'): requires ' + shiftRequirement.amount + " " + DISPLAY_NAMES[shiftRequirement.tier] + " Dimensions"
         else document.getElementById("resetLabel").textContent = 'Dimension Supersonic ('+ player.resets +'): requires ' + shiftRequirement.amount + " " + DISPLAY_NAMES[shiftRequirement.tier] + " Dimensions"
    ...
    if (canBuyTickSpeed() || player.currentEternityChall == "eterc9") {
        ...
        if (tickmult < 1e-9) document.getElementById("tickLabel").textContent = "Divide the tick interval by " + shortenDimensions(new Decimal(1).div(tickmult)) + '.'
        else {
            var places = 0
    ...
    if (document.getElementById("infinity").style.display == "block") {
        if (document.getElementById("preinf").style.display == "block") {
            ...
            if (player.tickSpeedMultDecreaseCost >= 1e12) document.getElementById("postinfi31").innerHTML = "Tickspeed cost multiplier increase <br>"+player.tickSpeedMultDecrease+"x"
            ...
            if (player.dimensionMultDecreaseCost >= 7e33) document.getElementById("postinfi42").innerHTML = "Dimension cost multiplier increase <br>"+player.dimensionMultDecrease.toFixed(2)+"x"
    ...
    if (document.getElementById("eternityupgrades").style.display == "block" && document.getElementById("eternitystore").style.display == "block") {
		...
		document.getElementById("eter7").innerHTML = "Dilated time gain is boosted by antimatter<br>Currently: "+shorten(1 + Math.log10(Math.max(1, player.money.log10())) / 40)+"x<br>Cost: "+shortenCosts(new Decimal("1e700000"))+" EP"
        document.getElementById("eter8").innerHTML = "Dilated time gain is boosted by infinity points<br>Currently: "+shorten(1 + Math.log10(Math.max(1, player.infinityPoints.log10())) / 20)+"x<br>Cost: "+shortenCosts(new Decimal("1e700000"))+" EP"
        document.getElementById("eter9").innerHTML = "Dilated time gain is boosted by eternity points<br>Currently: "+shorten(1 + Math.log10(Math.max(1, player.eternityPoints.log10())) / 10)+"x<br>Cost: "+shortenCosts(new Decimal("1e700000"))+" EP"
    ...
    if (document.getElementById("dilation").style.display == "block") {
        if (player.dilation.active) {
            if (Math.pow(Decimal.log10(player.money) / 400, TPExponent()) * (Math.pow(3, player.dilation.rebuyables[3])) - player.dilation.totalTachyonParticles <= 0) {
                document.getElementById("enabledilation").innerHTML = "Disable dilation.<br>Reach " + shortenMoney(Decimal.pow(10, Math.pow(player.dilation.tachyonParticles * Math.pow(400, TPExponent()) / Math.pow(3, player.dilation.rebuyables[3]), 1/TPExponent()))) + " antimatter to gain more Tachyon Particles."
            } else {
...
function updateEternityUpgrades() {
    ...
    document.getElementById("eter7").className = (player.eternityUpgrades.includes(7)) ? "eternityupbtnbought" : (player.eternityPoints.gte("1e700000")) ? "eternityupbtn" : "eternityupbtnlocked"
    document.getElementById("eter8").className = (player.eternityUpgrades.includes(8)) ? "eternityupbtnbought" : (player.eternityPoints.gte("1e700000")) ? "eternityupbtn" : "eternityupbtnlocked"
    document.getElementById("eter9").className = (player.eternityUpgrades.includes(9)) ? "eternityupbtnbought" : (player.eternityPoints.gte("1e700000")) ? "eternityupbtn" : "eternityupbtnlocked"
...
document.getElementById("postinfi31").onclick = function() {
    if (player.infinityPoints.gte(player.tickSpeedMultDecreaseCost) && player.tickSpeedMultDecreaseCost < 1e12) {
...
document.getElementById("postinfi42").onclick = function() {
    if (player.infinityPoints.gte(player.dimensionMultDecreaseCost) && player.dimensionMultDecreaseCost < 7e33) {
...
function updateInfCosts() {
    ...
    if (document.getElementById("timestudies").style.display == "block" && document.getElementById("eternitystore").style.display == "block") {
        document.getElementById("11desc").textContent = "Currently: "+shortenMoney(TS11Mult())+"x"
        ...
        document.getElementById("73desc").textContent = "Currently: "+shortenMoney(TS73Mult())+"x"
        ...
        document.getElementById("metaCost").textContent = shortenCosts(1e21);
...
function replicantiGalaxyAutoToggle() {
    if (player.replicanti.galaxybuyer) {
        ...
        if (player.timestudy.studies.includes(131) && !player.timestudy.studies.includes(1005)) document.getElementById("replicantiresettoggle").textContent = "Auto galaxy OFF (disabled)"
        else document.getElementById("replicantiresettoggle").textContent = "Auto galaxy OFF"
    } else {
        player.replicanti.galaxybuyer = true
        if (player.timestudy.studies.includes(131) && !player.timestudy.studies.includes(1005)) document.getElementById("replicantiresettoggle").textContent = "Auto galaxy ON (disabled)"
...
document.getElementById("reset").onclick = function () {
    if (forceHardReset) {
        if (window.location.href.split("//")[1].length > 20) set_save('challengingPlusSave', currentSave, defaultStart);
        else set_save('challengingPlusSave', currentSave, defaultStart);
        player = defaultStart
        infDimPow = 1;
        save_game();
        load_game();
...
function eternity(force, auto) {
    if ((player.infinityPoints.gte(Number.MAX_VALUE) && (!player.options.eternityconfirm || auto || confirm("Eternity will reset everything except achievements and challenge records. You will also gain an Eternity point and unlock various upgrades."))) || force === true) {
        ...
        if (player.currentEternityChall !== "") {
            ...
            if (player.currentEternityChall == "eterc6") player.dimensionMultDecrease = 10-Math.round(Math.log10(player.dimensionMultDecreaseCost/100000000)/Math.log10(5000))-eterc6Reward();
			if (player.currentEternityChall == "eterc11") player.tickSpeedMultDecrease = 10-Math.round(Math.log10(player.tickSpeedMultDecreaseCost/3e6)/Math.log10(5))-eterc11Reward();
			player.etercreq = 0
            respecTimeStudies()
			...
			if(player.dilation.studies.includes(16)){
				var ec=parseInt(player.currentEternityChall.slice(5));
				player.challengingMatter[ec]=player.challengingMatter[ec].plus(gainedCM());
			}
        }
        for (var i=0; i<player.challenges.length; i++) {
        ...
        if (player.dilation.active && (!force || player.infinityPoints.gte(Number.MAX_VALUE))) {
            let tachyonGain = Math.max(Math.pow(Decimal.log10(player.money) / 400, TPExponent()) * (Math.pow(3, player.dilation.rebuyables[3])) - player.dilation.totalTachyonParticles, 0)
        ...
        player = {
            ...
            infMult: new Decimal(1),
            infMultCost: new Decimal(10),
            tickSpeedMultDecrease: player.eternities > 18 ? player.tickSpeedMultDecrease : 10-eterc11Reward(),
            tickSpeedMultDecreaseCost: player.eternities > 18 ? player.tickSpeedMultDecreaseCost : 3e6,
            dimensionMultDecrease: player.eternities > 18 ? player.dimensionMultDecrease : 10-eterc6Reward(),
...
function buyDilationUpgrade(id, costInc) {
    if (id > 3 && id != 25 && id != 11) { // Not rebuyable
		...
		if (id == 25) {
			player.timestudy.theorem += (player.timestudy.amcost.e / 20000 - 1)*(dilationTTMult1(player.dilation.rebuyables[25])-dilationTTMult1(player.dilation.rebuyables[25]-1));
		}
    }
...
if(player.dilation.studies.includes(16))document.getElementById("challmatter").style.display = "block";
	for(var i=1;i<13;i++){
		...
		if(ECTimesCompleted("eterc"+i) == 15){
			document.getElementById("eterc"+i+"goal").textContent = "";
			//challenging matter
			if(player.dilation.studies.includes(16)){
				document.getElementById("eterc"+i+"goal").textContent = "EC"+i+" Challenging Matter: "+shortenMoney(player.challengingMatter[i]);
			}
		}
...
function gameLoop(diff) {
    ...
    if (player.replicanti.galaxybuyer && player.replicanti.amount.gte(Number.MAX_VALUE) && (!player.timestudy.studies.includes(131) || player.timestudy.studies.includes(1005))) {
    ...
    if (player.dilation.active) document.getElementById("eternitybtn").innerHTML = "Gain <b>"+shortenDimensions(gainedEternityPoints())+"</b> Eternity points.<br>"+"+"+shortenMoney(Math.round(Math.max(Math.pow(Decimal.log10(player.money) / 400, TPExponent()) * (Math.pow(3, player.dilation.rebuyables[3])) - player.dilation.totalTachyonParticles, 0) * 10)/10) +" Tachyon particles."
	...
	if (player.currentEternityChall !== "" && (ECTimesCompleted(player.currentEternityChall) == 15 || ECTimesCompleted(player.currentEternityChall) == 14) && player.dilation.studies.includes(16)){
		document.getElementById("eternitybtn").innerHTML = "Other challenges await.. I need to become Eternal<br>"+"+"+shortenMoney(gainedCM())+" EC"+player.currentEternityChall.slice(5)+" Challenging Matter."
	}
    updateMoney();
    updateCoinPerSec();
    updateDimensions()
    ...
    if (player.tickSpeedMultDecreaseCost >= 1e12) document.getElementById("postinfi31").className = "infinistorebtnbought"
    ...
    if (player.dimensionMultDecreaseCost >= 7e33) document.getElementById("postinfi42").className = "infinistorebtnbought"
    ...
    document.getElementById("ec4reward").textContent = "Reward: Infinity Dimension multiplier from unspent IP, Currently: "+shortenMoney(eterc4Mult())+"x"
    document.getElementById("ec5reward").textContent = "Reward: Distant Galaxy cost scaling starts "+(eterc5R1())+" galaxies later. Remote Galaxy cost scaling starts "+(eterc5R2r())+" galaxies later."
    document.getElementById("ec6reward").textContent = "Reward: Further reduce the dimension cost multiplier increase, Currently: "+player.dimensionMultDecrease.toFixed(2)+"x "
    ...
    document.getElementById("ec8reward").textContent = "Reward: Infinity power powers up replicanti galaxies, Currently: " + (eterc8Reward() * 100).toFixed(2) + "%"
    document.getElementById("ec9reward").textContent = "Reward: Infinity Dimension multiplier based on time shards, Currently: "+shortenMoney(eterc9Mult())+"x "
    document.getElementById("ec10reward").textContent = "Reward: Time dimensions gain a multiplier from infinitied stat, Currently: "+shortenMoney(new Decimal(Math.max(Math.pow(getInfinitied(), 0.9) * EC10Reward1() * 0.000002+1, 1)).pow((player.timestudy.studies.includes(31)) ? 4 : 1).pow(EC10Reward2()))+"x "
    ...
    if (player.timestudy.studies.includes(194)) player.eternityPoints = player.eternityPoints.plus(gainedEternityPoints().times(diff/1000))
